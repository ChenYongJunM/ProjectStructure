...
@synthesize gogoActivityView = _gogoActivityView;
...
//        _slime.startPoint = CGPointMake(frame.size.width / 2, height / 2);
        _slime.translatesAutoresizingMaskIntoConstraints = NO;
        [self addSubview:_slime];
//        [self addConstraint:[NSLayoutConstraint constraintWithItem:_slime attribute:NSLayoutAttributeCenterY relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeCenterY multiplier:1 constant:0]];
//        [self addConstraint:[NSLayoutConstraint constraintWithItem:_slime attribute:NSLayoutAttributeCenterX relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeCenterX multiplier:1 constant:0]];
...
//        _gogoActivityView = [[GOGOActivityIndicatorView alloc] init];
//        [_gogoActivityView stopDoAnimation];
////        _gogoActivityView.center = _slime.startPoint;
//        _gogoActivityView.center = CGPointMake(_slime.startPoint.x, _slime.startPoint.y + 1);
//        _gogoActivityView.hidden = YES;
//        [self addSubview:_gogoActivityView];
        
        [_slime setPullApartTarget:self
                            action:@selector(pullApart:)];
        _dragingHeight = height - 8;
    }
    return self;
}
...
//        _slime.startPoint = CGPointMake(frame.size.width / 2, _dragingHeight / 2);
    ...
    _refleshView.center = _slime.startPoint;
//    _activityIndicatorView.center = CGPointMake(_slime.startPoint.x, _slime.startPoint.y + 7) ;
//    _gogoActivityView.center = CGPointMake(_slime.startPoint.x, _slime.startPoint.y + 7) ;
}
...
//        [_gogoActivityView startDoAnimation];
        if (_gogoActivityView) {
            _gogoActivityView = nil;
            [_gogoActivityView removeFromSuperview];
        }
        _gogoActivityView = [[GOGOActivityIndicatorView alloc] init];
        [_gogoActivityView stopDoAnimation];
        _gogoActivityView.origin = CGPointMake((ScreenWidth-_gogoActivityView.width)/2, 20);
        [self addSubview:_gogoActivityView];
        
        
//        _gogoActivityView.translatesAutoresizingMaskIntoConstraints = NO;
//        [self addConstraint:[NSLayoutConstraint constraintWithItem:_gogoActivityView attribute:NSLayoutAttributeCenterY relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeCenterY multiplier:1 constant:0]];
//        [self addConstraint:[NSLayoutConstraint constraintWithItem:_gogoActivityView attribute:NSLayoutAttributeCenterX relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeCenterX multiplier:1 constant:0]];
